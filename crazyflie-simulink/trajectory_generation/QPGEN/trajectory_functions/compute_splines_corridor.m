function [ P ] = compute_splines_corridor( points , times , N , cost , handles)
    ...
    %% Adds corridor constraints
    Aineq = [];
    bineq = [];
    for ii = 1:size(handles, 1)
        if ~isnan(handles{ii,3})
            numpoints = handles{ii,3};
            tt  = linspace(0, T(ii), numpoints);
            A_temp = zeros(numpoints, (N+1)*nS);
            for jj = 1:N+1
                A_temp(:,(N+1)*(ii-1)+jj) = tt.^(jj-1);
            end
            Aineq = [Aineq; A_temp; -A_temp];
            bineq = [bineq; handles{ii,1}(tt)'; -handles{ii,2}(tt)'];
        end
    end
    ...
    [P, ~, ~, ~, ~] = quadprog(Q, [], Aineq, bineq, Aeq, beq, [], [], x0, opt);
end
