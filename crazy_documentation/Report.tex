...
The control and development of small autonomous aircraft is a very interesting problem with myriad applications. In previous research, small rotorcraft have been used to build temporary rope bridges between gorges, with the mindset of eventually aiding aid fire brigades and rescue workers~\cite{gglr2015}. Other interesting but slightly less useful examples include a quadcopters which detect the trajectories of a thrown balls to bounce it back to the tosser. When it comes to potential applications the sky is quite literally the limit.
...
All code is available and up to date on \texttt{https://github.com/mgreiff/crazyflie\_project}, complete with examples for testing and a README.md for installation and dependency requirements. Every plot can be reproduced from data and/or examples in the Github repository.
...
\newpage\subsection{Linearised dynamics}\label{sec:linearized}
...
To estimate the states properly, many approaches are considered and compared in simulation before making a real time implementation. As we know from previous sections, the positions and velocities in the x- and y-directions are unobservable in the considered non-linear dynamics. Since these four states are not needed in stabilising the quadcopter, our approach will be to estimate the remaining eight states on the quadcopter and run this in conjunction with a stabilising inner controller (discussed above). This inner loop will be run at $\approx 500-100$ Hz and would preferably run completely independent of external control and estimation. In order to estimate the four unobservable states, some means of motion capture will be run on the host computer. In the current implementation, a Kinect 1 camera is used, yielding positional measurements at $\approx 30$ Hz. If the external measurements prove to bad to accurately determine the unobservable states, the Inertial Measurement Unit (IMU) data could be used in the outer loop as well while running a complementary filter on the crazyflie. However, all data will be delayed when sent to the host computer from the crazyflie via bluetooth, and likewise, the estimated positions and speeds will suffer from a time delay when transmitted back to the crazyflie.
...
\end{equation}
given the torques and thrusts or the rotor angular speeds, subject to CPU constraints and the highly non-linear dynamics of the crazyflie. To achieve this, a regular Kalman filter (KF) is tested, using the dynamics linearised around the stable hovering state to compute the filter gain ~\cite{glad2000control}. In addition the extended Kalman filter (EKF) is tested. The EKF effectively linearises the system around the previous state estimate on each time step, which is done by computing an expression for the Jacobian offline and then evaluating it on-line. However, as the dynamics contain non-linear matrix inverses, this puts great constraints on how fast the inner loop can be run. Two other model based approaches, the unscented Kalman filter (UKF) and generic particle filter (GPF), are implemented and tested on the non-linear dynamics. If the CPU is too restrictive to use any of the above mentioned filters, one could consider running the heavy estimation on the host computer and simply use a complementary filter on the quadcopter.
...
\subsection{Complementary filter (CF)}
