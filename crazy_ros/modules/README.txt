This entire directory defines the crazylib module and sets up the necessary c-wrappers
for running a simple sum example (using the c-code in /modules/sum/sum.c) and
implementing the cvxgen solver (located in /modules/cvxgen/*). Note that all the
custom scripts in '/cvxgen/* are prefixed with ROS, everything elese is as it was
generated from the makefile. The c-code is compiled as dynamical libraries (.so) and
included in the crazylib module using cython.

Currently, the module contains
1)  An example function for computing the sum of integers in a list, showing how the
